...
def head_pose(vid):
        ...
        while True:
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    if not (-7<int(y)<7 and -7<int(x)<7): 
                        straight = 0
                        text = 'Head not straight'
                ...
                if 0<=((cont[0]+rcont[0])/2)<=3 and 0<=((cont[1]+rcont[1])/2)<=3.5:
                    contact = True
                else:
                    contact = False
                '''
    ...
    if head_score<=50:
        message = "Your head was not straight most of the time. Keep it straight."
    elif 50<head_score<=90:
        message = "Consider maintaining a more consistent straight head posture."
    elif 90<head_score:
        message = "Great job maintaining your head straight! It showcases your focus and attentiveness."

    if blink_too_long == 1:
        message =  message + " Don't close your eyes for too long."

    if eye_score<=25:
        message = message + " It seems like you are looking away occasionally. Consider practicing maintaining eye contact."
    elif 25<eye_score<=50:
        message = message + " Limited eye contact detected. Consider practicing maintaining eye contact for longer stretches to increase your confidence and connect with your audience."
    elif 50<eye_score<=75:
        message = message + " Your eye contact is not bad, but try holding it longer."
    elif 75<eye_score<=90:
        message = message + " Good Job in maintaining eye contact for most of the time."
    elif 90<eye_score:
        message = message + " Impressive! Your eye contact is very strong!"
    ...
    try:
        if total_blink>20:
            message = message + " You are blinking too much. On average, most people blink around 15 to 20 times each minute. You were blinking on an average of " + str(int(total_blink)) + ". Too much blinking indicate lack of concentration."
    except:
        print('')
