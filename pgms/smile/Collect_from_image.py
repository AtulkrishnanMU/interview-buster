...
def logging_csv(number, landmark_list):
    if 0 <= number <= 5:
        csv_path = r'E:\website files\bodylang\myapp\pgms\smile\model\keypoint_classifier\keypoint.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list])
    return

root = "E:/website files/emotions(copy)/"
IMAGE_FILES = []
category = ['laugh','grin','neutral', 'open-mouth']
#laugh - open-mouth smile
#grin - closed mouth smile
#neutral - closed mouth not smile
#open-mouth - open mouth not smile
